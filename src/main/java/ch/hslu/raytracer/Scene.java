...
public class Scene {
    ...
    private Color calculateColor(HitInfo hitInfo) {
        ...
        Color objectColor = hitInfo.getObject().getColor();
        ...
        for (Light light : lights) {
            ...
            Vector lightDirection = light.position().subtract(hitInfo.getHitPoint()).normalize();

            // Calculate diffuse lighting using Lambert's cosine law
            double diffuseFactor = Math.max(0, hitInfo.getNormal().dot(lightDirection));

            // Check for shadows
            boolean inShadow = isInShadow(hitInfo.getHitPoint(), lightDirection);
            if (!inShadow) {
